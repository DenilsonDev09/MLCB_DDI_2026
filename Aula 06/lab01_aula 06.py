...
from sklearn.tree import DecisionTreeClassifier   #classe responsável pela matemática propabilística
...
def extrair_sentence_embedding(texto_limpo: str, modelo_emb) -> np.ndarray:
    ...
    return np.max(vetores, axis=0) #: Calcula a média matemática de cada uma das 50 dimensões entre os vetores de todas as palavras da frase.
# O axis=0 instrui o NumPy a calcular a média coluna por coluna (ao longo das linhas), e não da matriz inteira.
...
LIMIAR_CONFIANCA = 0.65  # Limiar de corte para aceitação da resposta do bot
...
def processar_atendimento_sac(mensagem_usuario: str):
    ...
    # Step 4: Regra de Decisão / Fallback
    if confianca >= LIMIAR_CONFIANCA:
        classificacao_status = f" IDENTIFICADO ({intencao_detectada}) (Corte mínimo: 65%)"
        texto_resposta = RESPOSTAS_PADRAO[intencao_detectada]
    else:
        classificacao_status = " UNCERTAIN (Fallback Acionado) (Corte mínimo: 65%)"
        texto_resposta = (
            "Desculpe, não consegui compreender com clareza a sua solicitação. "
            "Estou transferindo agora mesmo sua conversa para um de nossos atendentes. Por favor, aguarde um momento."
        )
...
dados_imobiliaria = [
    ...
    ("Como faço para encerrar o contrato do imóvel?", "cancelar_contrato"),
    ("Gostaria de solicitar o cancelamento do meu contrato", "cancelar_contrato"),
    ("Quais são os procedimentos para rescindir o contrato de locação?", "cancelar_contrato"),
    ("Preciso cancelar o contrato antes do prazo, como devo proceder?", "cancelar_contrato")
]
...
RESPOSTAS_PADRAO = {
    ...
    "cancelar_contrato": (
    "**Cancelamento de Contrato:** Para solicitar o encerramento do seu contrato, "
    "acesse a Área do Cliente em www.imobiliaria.com/cliente e selecione a opção "
    "\"Solicitar Cancelamento de Contrato\". Consulte as condições de rescisão e, "
    "se necessário, envie os documentos solicitados para análise da equipe responsável."
    )
